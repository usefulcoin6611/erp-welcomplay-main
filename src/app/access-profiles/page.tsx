...
export default function AccessProfilesPage() {
  ...
  const fetchRoles = useCallback(async () => {
    ...
    try {
      ...
      if (!res.ok || !json?.success) {
        toast.error(json?.message ?? 'Failed to load access profiles')
        return
      }
      const list = Array.isArray(json.data) ? json.data : []
      setRoles(
        list.map((r: { id: string; name: string; permissions: unknown }) => ({
          id: r.id,
          name: r.name,
          permissions: normalizePermissions(Array.isArray(r.permissions) ? r.permissions : []),
        }))
      )
    } catch (err) {
      console.error(err)
      toast.error('Failed to load access profiles')
    } finally {
      setLoading(false)
    }
  }, [])
      ...
      fetch('/api/branches')
        ...
        .then((j) => {
          if (j?.data && Array.isArray(j.data)) {
            setBranches(j.data.map((b: { id: string; name: string }) => ({ id: b.id, name: b.name })))
          }
        })
        .catch(() => {})
    }
  ...
  const handleCreateSubmit = async (e: React.FormEvent) => {
    ...
    if (isSuperAdmin && !editRole && !formData.branchId?.trim()) {
      toast.error('Branch is required when creating a profile as super admin')
      return
    }
    setSaving(true)
    try {
      ...
      if (editRole) {
        const res = await fetch(`/api/access-profiles/${editRole.id}`, {
          method: 'PUT',
        ...
        const json = await res.json().catch(() => null)
        if (!res.ok || !json?.success) {
          toast.error(json?.message ?? 'Failed to update access profile')
          return
        }
        ...
        const json = await res.json().catch(() => null)
        if (!res.ok || !json?.success) {
          toast.error(json?.message ?? 'Failed to create access profile')
          return
        }
      ...
      console.error(err)
      toast.error(editRole ? 'Failed to update access profile' : 'Failed to create access profile')
    } finally {
      setSaving(false)
    }
  }
  ...
  const handleDelete = async (id: string) => {
    ...
    setSaving(true)
    try {
      const res = await fetch(`/api/access-profiles/${id}`, { method: 'DELETE' })
      const json = await res.json().catch(() => null)
      if (!res.ok || !json?.success) {
        toast.error(json?.message ?? 'Failed to delete access profile')
        return
      }
      ...
      console.error(err)
      toast.error('Failed to delete access profile')
    } finally {
      setSaving(false)
    }
  }
      ...
      <div className="space-y-4">
        <div className="flex items-center justify-between">
          <h6 className="text-sm font-semibold">
            {category === 'staff' && 'Assign General Permission to Access Profiles'}
            {category === 'crm' && 'Assign CRM related Permission to Access Profiles'}
            {category === 'project' && 'Assign Project related Permission to Access Profiles'}
            {category === 'hrm' && 'Assign HRM related Permission to Access Profiles'}
            {category === 'account' && 'Assign Account related Permission to Access Profiles'}
            {category === 'pos' && 'Assign POS related Permission to Access Profiles'}
          </h6>
        </div>
          ...
          <div className="@container/main flex flex-1 flex-col gap-4 p-4 bg-gray-100">
            <Card className="shadow-[0_1px_2px_0_rgb(0_0_0_/_0.03)]">
              <CardHeader className="px-6 flex flex-row items-center justify-between gap-4">
                <div className="min-w-0 space-y-1 flex-1">
                  <CardTitle className="text-lg font-semibold">Manage Access Profiles</CardTitle>
                  <CardDescription>
                    Create and manage access profiles for employees. Each profile is a set of permissions (e.g. HR, Sales, Accountant). Assign a profile to a user when creating or editing them under Users. This is separate from user type (Employee, Client).
                  </CardDescription>
                </div>
                <div className="flex flex-wrap items-center gap-2">
                  <Dialog open={dialogOpen} onOpenChange={handleDialogOpenChange}>
                    <DialogTrigger asChild>
                      <Button variant="blue" size="sm" className="shadow-none h-7 w-7 p-0" title="Create Access Profile">
                    ...
                    <DialogContent className="!max-w-[85vw] w-[85vw] h-[90vh] max-h-[95vh] overflow-hidden p-0 flex flex-col">
                      <div className="border-b px-6 py-4">
                        <DialogHeader>
                          <DialogTitle>{editRole ? 'Edit Access Profile' : 'Create Access Profile'}</DialogTitle>
                        </DialogHeader>
                      </div>
                      <form onSubmit={handleCreateSubmit} className="flex flex-1 flex-col overflow-hidden px-6 pb-5">
                        <div ref={permissionsScrollRef} className="grid flex-1 gap-4 overflow-y-auto content-start">
                          <div className="space-y-2">
                            ...
                            <Input
                              ...
                              placeholder="e.g. HR Manager, Accountant, Sales"
                              required
                            />
                          </div>
                          {isSuperAdmin && !editRole && (
                            <div className="space-y-2">
                              ...
                              <Select
                                ...
                                onValueChange={(v) => setFormData({ ...formData, branchId: v === '_none' ? '' : v })}
                              >
                                <SelectTrigger>
                        ...
                        <DialogFooter className="pt-4">
                          ...
                          <Button type="submit" variant="blue" disabled={saving}>{saving ? 'Saving...' : editRole ? 'Update' : 'Create'}</Button>
                        </DialogFooter>
                      </form>
                    </DialogContent>
                  </Dialog>
            ...
            <Card className="shadow-[0_1px_2px_0_rgb(0_0_0_/_0.03)]">
              <CardContent className="p-0">
                {loading ? (
                  <div className="p-8 text-center text-muted-foreground">Loading access profiles...</div>
                ) : (
                <div className="overflow-x-auto">
                  <Table>
                    <TableHeader>
                      <TableRow>
                        <TableHead className="px-6">Access Profile</TableHead>
                        ...
                        <TableRow key={role.id}>
                          ...
                          <TableCell className="px-6">
                            <div className="flex items-center gap-2">
                              ...
                              {role.name !== 'Employee' && role.name !== 'Default' && (
                                <Button
                                  variant="outline"
                                  size="sm"
