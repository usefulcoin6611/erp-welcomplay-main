...
export async function PUT(
  ...
  try {
    ...
    const awardType = await (prisma as any).awardType.update({
      where: { id },
      data: {
        name,
      },
    ...
    return NextResponse.json({
      success: true,
      message: "Award type berhasil diperbarui",
      data: awardType,
    });
  } catch (error) {
    console.error("Error updating award type:", error);
    return NextResponse.json(
      { success: false, message: "Gagal memperbarui award type" },
      { status: 500 }
    );
  }
}
...
export async function DELETE(
  ...
  try {
    ...
    await (prisma as any).awardType.delete({
      where: { id },
    });

    return NextResponse.json({
      success: true,
      message: "Award type berhasil dihapus",
    });
  } catch (error) {
    console.error("Error deleting award type:", error);
    return NextResponse.json(
      { success: false, message: "Gagal menghapus award type" },
      { status: 500 }
    );
  }
}
