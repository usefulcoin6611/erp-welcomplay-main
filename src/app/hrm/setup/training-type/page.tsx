...
export default function ManageTrainingTypePage() {
  ...
  const [trainingTypeToDelete, setTrainingTypeToDelete] =
  ...
  const fetchTrainingTypes = async () => {
    ...
    try {
      ...
      if (data.success) {
        ...
        toast.error(data.message || 'Gagal memuat training type');
      }
    } catch (error) {
  ...
  const handleEditClick = (item: TrainingType) => {
    setDialogOpen(true);
    setEditingId(item.id);
    setName(item.name);
  };

  const openDeleteConfirm = (item: TrainingType) => {
    setTrainingTypeToDelete(item);
    setDeleteAlertOpen(true);
  };

  const handleConfirmDelete = async () => {
    if (!trainingTypeToDelete) return;
    setIsSubmitting(true);
    try {
      const res = await fetch(
        `/api/training-types/${trainingTypeToDelete.id}`,
        {
          method: 'DELETE',
        }
      );
      const data = await res.json();
      if (data.success) {
        toast.success(data.message || 'Training type berhasil dihapus');
        fetchTrainingTypes();
      } else {
        toast.error(data.message || 'Gagal menghapus training type');
      }
    } catch (error) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!name.trim()) {
      toast.error('Nama training type wajib diisi');
      return;
    }
    ...
    try {
      ...
      if (data.success) {
        toast.success(
          data.message ||
            (editingId
              ? 'Training type berhasil diperbarui'
              : 'Training type berhasil dibuat')
        );
        setDialogOpen(false);
        setEditingId(null);
        setName('');
        ...
        toast.error(data.message || 'Gagal menyimpan training type');
      }
    } catch (error) {
  ...
  const filteredTrainingTypes = useMemo(
    () =>
      trainingTypes.filter((tt) =>
        tt.name.toLowerCase().includes(searchTerm.trim().toLowerCase())
      ),
  ...
  const paginatedTrainingTypes = useMemo(() => {
    ...
    return filteredTrainingTypes.slice(start, start + pageSize);
  }, [filteredTrainingTypes, currentPage, pageSize]);
    ...
    <>
              ...
              <div>
                ...
                <Breadcrumb>
                  <BreadcrumbList className="text-muted-foreground">
                    ...
                    <BreadcrumbSeparator />
                    <BreadcrumbItem>
                      <BreadcrumbPage>Training Type</BreadcrumbPage>
                    </BreadcrumbItem>
                  </BreadcrumbList>
                </Breadcrumb>
              </div>
            ...
            <div className="flex items-center justify-between border-t border-gray-200/80 pt-3 px-2 text-xs text-muted-foreground">
              ...
              <div className="flex items-center gap-2">
                ...
                <Button
                  ...
                  disabled={
                    currentPage ===
                    Math.ceil(filteredTrainingTypes.length / pageSize)
                  }
                  onClick={() =>
                    setCurrentPage((prev) =>
                      Math.min(
                        Math.ceil(filteredTrainingTypes.length / pageSize) || 1,
                        prev + 1
                      )
                    )
                  }
      ...
      <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
        <DialogContent className="sm:max-w-[400px]">
          <DialogHeader>
            <DialogTitle>
              {editingId ? 'Edit Training Type' : 'Add Training Type'}
            </DialogTitle>
            <DialogDescription>
              {editingId
                ? 'Perbarui informasi training type.'
                : 'Tambahkan training type baru untuk HRM system.'}
            </DialogDescription>
          </DialogHeader>
          <form onSubmit={handleSubmit} className="space-y-4">
            <div className="space-y-2">
              <label
                htmlFor="training-type-name"
      ...
      <AlertDialog open={deleteAlertOpen} onOpenChange={setDeleteAlertOpen}>
        <AlertDialogContent>
          <AlertDialogHeader>
            <AlertDialogTitle>Hapus training type?</AlertDialogTitle>
            <AlertDialogDescription>
              Tindakan ini tidak dapat dibatalkan. Data training type akan
