...
export default function ManageLeaveTypePage() {
  ...
  const fetchLeaveTypes = async () => {
    ...
    try {
      ...
      if (data.success) {
        ...
        toast.error(data.message || 'Gagal memuat data leave type');
      }
    } catch (error) {
  ...
  const handleConfirmDelete = async () => {
    ...
    try {
      const res = await fetch(`/api/leave-types/${leaveTypeToDelete.id}`, {
        method: 'DELETE',
      });
      const data = await res.json();
      if (data.success) {
        toast.success(data.message || 'Leave type berhasil dihapus');
        fetchLeaveTypes();
      } else {
        toast.error(data.message || 'Gagal menghapus leave type');
      }
    } catch (error) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (data.success) {
        toast.success(
          data.message ||
            (editingId ? 'Leave type berhasil diperbarui' : 'Leave type berhasil dibuat')
        );
        setDialogOpen(false);
        setEditingId(null);
        setName('');
  ...
  const filteredLeaveTypes = useMemo(
    () =>
      leaveTypes.filter((lt) =>
        lt.name.toLowerCase().includes(searchTerm.trim().toLowerCase())
      ),
  ...
  return (
    <>
        ...
        <CardContent className="px-6 py-4">
          <div className="flex flex-wrap items-center justify-between gap-4">
            <div className="space-y-2">
              <div>
                <h4 className="mb-2 text-xl font-semibold">Manage Leave Type</h4>
                <Breadcrumb>
      ...
      >
        <DialogContent className="sm:max-w-[425px]">
          <DialogHeader>
            ...
            <DialogDescription>
              {editingId
                ? 'Perbarui informasi leave type.'
                : 'Tambahkan leave type baru.'}
            </DialogDescription>
          </DialogHeader>
      ...
      >
        <AlertDialogContent>
          <AlertDialogHeader>
            <AlertDialogTitle>Hapus Leave Type?</AlertDialogTitle>
            <AlertDialogDescription>
              &quot;{leaveTypeToDelete?.name}&quot; akan dihapus. Tindakan ini tidak dapat
              dibatalkan.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
