...
export default function ManageLoanOptionPage() {
  ...
  const [optionToDelete, setOptionToDelete] = useState<LoanOption | null>(null);

  const total = loanOptions.length;

  const fetchLoanOptions = async () => {
    setIsLoading(true);
    try {
      const res = await fetch('/api/loan-options');
      const data = await res.json();
      if (data.success) {
        setLoanOptions(data.data);
      } else {
        toast.error(data.message || 'Gagal memuat loan option');
      }
    } catch (error) {
  ...
  const handleEditClick = (item: LoanOption) => {
    setDialogOpen(true);
  ...
  const openDeleteConfirm = (item: LoanOption) => {
  ...
  const handleConfirmDelete = async () => {
    ...
    try {
      const res = await fetch(`/api/loan-options/${optionToDelete.id}`, {
        method: 'DELETE',
      });
      const data = await res.json();
      if (data.success) {
        toast.success(data.message || 'Loan option berhasil dihapus');
        fetchLoanOptions();
      } else {
        toast.error(data.message || 'Gagal menghapus loan option');
      }
    } catch (error) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!name.trim()) {
      toast.error('Nama loan option wajib diisi');
      return;
    }
    ...
    try {
      ...
      if (data.success) {
        toast.success(
          data.message ||
            (editingId
              ? 'Loan option berhasil diperbarui'
              : 'Loan option berhasil dibuat')
        );
        setDialogOpen(false);
        setEditingId(null);
        setName('');
        ...
        toast.error(data.message || 'Gagal menyimpan loan option');
      }
    } catch (error) {
    ...
    <>
        ...
        <CardContent className="px-6 py-4">
          <div className="flex flex-wrap items-center justify-between gap-4">
            <div className="space-y-2">
              <div>
                <h4 className="mb-2 text-xl font-semibold">Manage Loan Option</h4>
                <Breadcrumb>
                  <BreadcrumbList className="text-muted-foreground">
                    ...
                    <BreadcrumbSeparator />
                    <BreadcrumbItem>
                      <BreadcrumbPage>Loan Option</BreadcrumbPage>
                    </BreadcrumbItem>
                  </BreadcrumbList>
                </Breadcrumb>
              </div>
      ...
      >
        <DialogContent className="sm:max-w-[480px]">
          <DialogHeader>
            <DialogTitle>
              {editingId ? 'Edit Loan Option' : 'Create New Loan Option'}
            </DialogTitle>
            <DialogDescription>
              {editingId
                ? 'Perbarui informasi loan option.'
                : 'Tambahkan loan option baru.'}
            </DialogDescription>
          </DialogHeader>
          <form onSubmit={handleSubmit}>
            <div className="grid gap-4 py-4">
              <div className="space-y-2">
                <Label htmlFor="loan-option-name">Name</Label>
                <Input
                  id="loan-option-name"
                  value={name}
              ...
              <div className="space-y-2">
                <Label htmlFor="loan-option-description">Description</Label>
                <Textarea
                  id="loan-option-description"
                  value={description}
      ...
      >
        <AlertDialogContent>
          <AlertDialogHeader>
            <AlertDialogTitle>Hapus Loan Option?</AlertDialogTitle>
            <AlertDialogDescription>
