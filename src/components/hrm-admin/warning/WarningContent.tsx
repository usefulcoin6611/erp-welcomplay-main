...
export function WarningContent() {
  ...
  const validateForm = (): boolean => {
    ...
    if (!formData.warningById?.trim()) {
      errors.warningById = 'Warning by is required';
    }
    if (!formData.employeeId?.trim()) {
      errors.employeeId = 'Warning to is required';
    }
    if (formData.warningById && formData.employeeId && formData.warningById === formData.employeeId) {
      errors.employeeId = 'Warning to must be different from warning by';
    }
    if (!formData.subject?.trim()) {
      errors.subject = 'Subject is required';
    }
    if (!formData.warningDate?.trim()) {
      errors.warningDate = 'Warning date is required';
    } else {
      const d = new Date(formData.warningDate);
      if (Number.isNaN(d.getTime())) {
        errors.warningDate = 'Invalid date';
      }
    }
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      const [warningsRes, employeesRes] = await Promise.all([
        fetch('/api/hrm/admin/warnings'),
        fetch('/api/hrm/admin/employees'),
      ]);
      const [warningsJson, employeesJson] = await Promise.all([
        warningsRes.json(),
        employeesRes.json(),
      ]);
      if (warningsJson?.success && Array.isArray(warningsJson.data)) setData(warningsJson.data);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setSaving(true);
    try {
      const url = editingId ? `/api/hrm/admin/warnings/${editingId}` : '/api/hrm/admin/warnings';
      const body = {
        employeeId: formData.employeeId.trim(),
        warningById: formData.warningById.trim(),
        subject: formData.subject.trim(),
        warningDate: formData.warningDate.trim(),
  ...
  const handleEdit = (d: Warning) => {
    setEditingId(d.id);
    setFormData({
      employeeId: d.employeeId,
      warningById: d.warningById,
      subject: d.subject,
  ...
  const handleDeleteConfirm = async () => {
    ...
    setSaving(true);
    try {
      const res = await fetch(`/api/hrm/admin/warnings/${idToDelete}`, { method: 'DELETE' });
      const json = await res.json().catch(() => null);
      if (!res.ok || !json?.success) {
    ...
    <div className="space-y-4">
          ...
          <CardContent className="pt-6">
            <div className="flex items-center justify-between">
              ...
              <div className={`w-10 h-10 rounded-lg flex items-center justify-center ${tabColor.iconBg}`}>
                <AlertTriangle className={`w-5 h-5 ${tabColor.iconText}`} />
              </div>
            </div>
          </CardContent>
        </Card>
          ...
          <CardContent className="pt-6">
            <div>
              ...
              <p className={`text-2xl font-bold ${tabColor.accent}`}>
                {data.filter((d) => d.warningDate.startsWith(thisMonth)).length}
              </p>
            </div>
          </CardContent>
        </Card>
        <Card className={statCardClass}>
          <CardContent className="pt-6">
            <div>
              ...
              <p className={`text-2xl font-bold ${tabColor.accent}`}>
                {data.filter((d) => d.warningDate.startsWith(thisYear)).length}
              </p>
            </div>
          </CardContent>
        </Card>
      ...
      <Card>
        ...
        <CardContent className="p-0">
          <Table>
            <TableHeader>
              <TableRow>
                <TableHead className="px-6">Warning By</TableHead>
                <TableHead className="px-6">Warning To</TableHead>
                <TableHead className="px-6">Subject</TableHead>
                <TableHead className="px-6">Warning Date</TableHead>
            ...
            <TableBody>
              {filteredData.length === 0 ? (
                ...
                filteredData.map((d) => (
                  <TableRow key={d.id}>
                    <TableCell className="px-6">{d.warningBy}</TableCell>
                    <TableCell className="px-6 font-medium">{d.employeeName}</TableCell>
                    <TableCell className="px-6">{d.subject}</TableCell>
                    <TableCell className="px-6">{d.warningDate}</TableCell>
      ...
      <Dialog open={formDialogOpen} onOpenChange={handleFormDialogOpenChange}>
        <DialogContent className="sm:max-w-lg max-h-[90vh] overflow-y-auto">
          <DialogHeader>
            <DialogTitle>{editingId ? 'Edit Warning' : 'Create New Warning'}</DialogTitle>
          </DialogHeader>
          <form onSubmit={handleSubmit} className="space-y-4">
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div className="space-y-2">
                <Label>Warning By <span className="text-destructive">*</span></Label>
                <Select
                  value={formData.warningById}
                  onValueChange={(v) => {
                    setFormData({ ...formData, warningById: v });
                    clearError('warningById');
                    if (formData.employeeId === v) clearError('employeeId');
                  }}
                >
                  <SelectTrigger className={formErrors.warningById ? 'border-destructive' : ''}>
                ...
                {formErrors.warningById && (
                  <p className="text-sm text-destructive">{formErrors.warningById}</p>
                )}
              </div>
              <div className="space-y-2">
                <Label>Warning To <span className="text-destructive">*</span></Label>
                <Select
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              ...
              <div className="space-y-2">
                <Label>Warning Date <span className="text-destructive">*</span></Label>
                <Input
                  type="date"
                  value={formData.warningDate}
                  onChange={(e: React.ChangeEvent<HTMLInputElement>) => {
                    setFormData({ ...formData, warningDate: e.target.value });
                    clearError('warningDate');
                  }}
                  className={formErrors.warningDate ? 'border-destructive' : ''}
                  aria-invalid={!!formErrors.warningDate}
                />
                {formErrors.warningDate && (
                  <p className="text-sm text-destructive">{formErrors.warningDate}</p>
                )}
              </div>
            </div>
            <div className="space-y-2">
