...
export function DealPipelineReport() {
  ...
  const fetchData = useCallback(async () => {
    setLoading(true)
    try {
      const res = await fetch('/api/crm/reports?type=deal')
  ...
  const pipelineChartOptions: Record<string, unknown> = {
    ...
    tooltip: { y: { formatter: (val: number) => `${val} deals` } },
    fill: { opacity: 1 },
    colors: ['#10B981'],
    legend: { position: 'top', horizontalAlign: 'left', offsetX: 40 }
  }

  const pipelineChartSeries = data?.series?.length ? data.series : [{ name: 'Deals', data: [0] }]
  ...
  return (
    <Card className="shadow-[0_1px_2px_0_rgba(0,0,0,0.04)] border-0 bg-white">
      <CardHeader className="px-5 pt-5 pb-3">
        ...
        <p className="text-sm text-muted-foreground mt-0.5">Deal distribution across pipeline stages</p>
      </CardHeader>
