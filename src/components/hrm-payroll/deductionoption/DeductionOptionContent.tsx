...
export function DeductionOptionContent() {
  const [searchTerm, setSearchTerm] = useState('');
  const [showForm, setShowForm] = useState(false);
  const [editingId, setEditingId] = useState<string | null>(null);
  const [formData, setFormData] = useState({ name: '', description: '' });
  const [data, setData] = useState([
    { id: '1', name: 'Tax Deduction', description: 'Income tax deduction', createdDate: '2024-01-15' },
    { id: '2', name: 'Insurance', description: 'Health insurance deduction', createdDate: '2024-01-20' },
  ]);
    ...
    <div className="space-y-4">
      <div className="flex justify-end items-center">
        <Button onClick={() => { setShowForm(true); setEditingId(null); setFormData({ name: '', description: '' }); }} className="bg-blue-500 hover:bg-blue-600 shadow-none">
          <Plus className="w-4 h-4 mr-2" />Add Deduction Option
        </Button>
      </div>
      {showForm && (
        <Card>
          <CardHeader><CardTitle>{editingId ? 'Edit' : 'Add'} Deduction Option</CardTitle></CardHeader>
          <CardContent>
