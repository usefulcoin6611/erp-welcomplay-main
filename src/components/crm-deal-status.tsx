...
interface CRMDealStatusProps {
  dealByStage?: { stage: string; count: number; percentage: number }[]
  loading?: boolean
}

export function CRMDealStatus({ dealByStage = [], loading = false }: CRMDealStatusProps) {
  const t = useTranslations("crmDashboard.dealStatus")
  const hasData = dealByStage.length > 0
